...
- One element per partial: when an element is restyled, it moves out of the shared file into its own `elements/_<element>.scss` (details and summary share one, since a summary only exists inside details; so do fieldset and legend). Load it from `main.scss` next to where its rules used to be, so the cascade order stays the same, and add it to the README project structure.
