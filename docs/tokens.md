...
2. `--lunar-light` and `--lunar-dark` are the main colors, used by `--lunar-bg`, `--lunar-fg`, `--lunar-border`, `--lunar-muted` and element styles. Derived colors are computed at runtime with CSS `color-mix(in oklch, …)`, not Sass, so user overrides of the main colors carry through. Always mix in `oklch`; `srgb` mixes are inconsistent with the rest of the theme. `--lunar-muted` mixes the background toward the text color by `--lunar-muted-mix` (default 60%).
